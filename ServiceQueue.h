...
public:
	...
	void update(int clock)
	{
		...
		if (!the_queue.empty()) {
			...
			//	if (plane->enter_departure_time == -1) { // new plane has arrived at the front
													   // update the enter_departure_time attribute of the plane
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				landing_queue -> total_wait = total_wait + wait;
				...
				//if (plane->start_service_time == -1) { // new plane has arrived at the front
				...
				*/
					plane->service_time= my_random.next_int(min_service_time);
